...
printf, lun, 'distance [pc], column density, exposure time [sec], number of lines combined, ESCAPE CSR median depth across line combos [%], ESCAPE CSR median depth uncertainty [%], ESCAPE CSR best detection [depth/uncertainty], ESCAPE CSR corresponding line combo, ESCAPE MIDEX median depth across line combos [%], ESCAPE MIDEX median depth uncertainty [%], ESCAPE MIDEX best detection [depth/uncertainty], ESCAPE MIDEX corresponding line combo'
free_lun, lun
      ...
      FOREACH expected_bg_event_ratio, expected_bg_event_ratios DO BEGIN
        FOREACH exposure_time_sec, exposure_time_secs DO BEGIN
          FOREACH num_lines_to_combine, num_lines_to_combines DO BEGIN
            ...
            escape_simulate_dimming, /NO_PLOTS, distance_pc=distance_pc, column_density=column_density, coronal_temperature_k=coronal_temperature_k, expected_bg_event_ratio=expected_bg_event_ratio, exposure_time_sec=exposure_time_sec, num_lines_to_combine=num_lines_to_combine, $
